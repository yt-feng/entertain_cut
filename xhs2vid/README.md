...
工作流 `.github/workflows/xhs-lowfan-kc-daily.yml` 每天北京时间 20:30 自动运行：

1. 8 个关键词 × 2 种排序 × 1 页，只搜索一次；对最多 20 位作者核验粉丝数。
2. 严格选择粉丝不超过 2 万、点赞不少于 200 的帖子；当天帖子优先，早间不足时由最近 26 小时补齐。
3. 单批次所有 TikHub 尝试共享一个 `90` 次硬计数器；失败重试也先计数，永远不会达到 100 次。
...
6. 成片完整解码并核验 1080×1920、H.264、AAC 后，上传并按远端大小复核到 `/我的坚果云/kc娱乐/YYYY-MM-DD/`。
