...
3. 每次运行会读取当天娱乐热点上下文（Tavily 优先，其他搜索源回退），最多替换 4 个搜索槽位；仍保留通用低粉关键词，并最多为 5 条中的 2 条增加“今日娱乐热点联动”卡片。热点不可用时自动回到原选题路径。
4. 补偿运行会先下载、校验并合并同一业务日的部分 Artifact，只生成仍缺的条目；TikHub 用量按来源 run 累计，硬上限始终为 `99`。
5. 每条成片最多使用 3 组一级评论与真实内嵌回复；封面及每段评论使用不同角色声线，超出六段时循环已验证声线而不丢整条批次。
6. 每条视频只向 APIMart 创建 1 张 `gpt-image-2 / 1K` 低价头像图集，本地派生所有评论头像；整批付费创建槽位不超过目标成片数（定时任务最多 5 次），候补超出后改用 Pillow 头像。
7. 成片完整解码并核验 1080×1920、H.264、AAC 后，上传并按远端大小复核到 `/我的坚果云/KC Desk Notes/Ops/YYYY-MM-DD/Portal 娱乐/`。常规娱乐 5 条和低粉爆款 5 条共用这一个日期目录；两条工作流都可通过仓库变量 `JIANGUOYUN_REMOTE_ROOT` 统一修改 `Ops` 根目录。常规工作流只清理 `KC娱乐_` 前缀的被替换成片，保留低粉视频。
8. 只有目标条数全部生成并上传成功，才更新跨日去重清单。
...
- 可选 `TAVILY_API_KEY`（当天娱乐热搜首选；未配置时回退到其他搜索源）
...
# 2. 抓素材: 下载笔记封面, app_v2 评论接口失败时回退 web_v3/web_v2，按点赞取评论
...
#    - 字幕逐页(jieba 词边界切分 ≤16 字), 保留句末标点、取消页间空格并增加可读停顿，自动高亮关键词
#    - 每页 macOS say(Tingting) TTS；角色声线降速并保留至少 0.85 秒阅读停留，避免还没说完就切到下一条
