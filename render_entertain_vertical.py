...
def render_static_overlay(plan: dict[str, Any], output: Path) -> None:
    ...
    draw_tag(draw, 744, 316, str(plan.get("side_badge", "表达太稳")), fill=(0, 205, 255, 224), text_fill=(8, 10, 18, 255), small=True)
    draw_tag(draw, 60, 1222, str(plan.get("caption_badge", "重点来了")), fill=(255, 214, 47, 236), text_fill=(13, 12, 18, 255), small=True)

    draw_sticker(draw, 812, 96, str(plan.get("sticker_top", "HOT")), str(plan.get("sticker_bottom", "气场")))
...
def draw_sticker(draw: ImageDraw.ImageDraw, x: int, y: int, top: str, bottom: str) -> None:
    ...
    draw.text((x + 28, y + 16), top, font=load_en_font(50, bold=True), fill=(14, 10, 20, 255), stroke_width=2, stroke_fill=(255, 255, 255, 220))
    draw.text((x + 42, y + 76), bottom, font=load_font(34, bold=True), fill=(255, 255, 255, 255), stroke_width=2, stroke_fill=(80, 0, 35, 220))
...
def build_filter_complex(
    ...
    *,
    ...
    if source_height > source_width:
        usable_height = max(2, source_height - min(vertical_bottom_crop, source_height - 2))
        usable_height -= usable_height % 2
        main_filter = (
            f"[mainsrc]crop={source_width}:{usable_height}:0:0,"
